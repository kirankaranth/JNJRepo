...
from XRay_Result_Parser.Models.attachment import Attachment
...
class XrayApiRequests:
    ...
    def build_test_execution_payload(self, test_execution_issue):
        ...
        test_execution_dict["info"]["startDate"] = test_execution_dict["tests"][0][
            "start"
        ]
        test_execution_dict["info"]["finishDate"] = test_execution_dict["tests"][-1][
            "finish"
        ]
    ...
    def _get_test_execution_details(self, test_execution_issue):
        ...
        description = (
            variables.EXECUTION_DESCRIPTION
        ...
        if test_execution_issue:
            ...
            if (
                description
                and updated_string in description
                and description.endswith(self.datetime_support.get_timezone_local)
            ):
                description = (
                    description.rsplit(updated_string, 1)[0]
                    + updated_string
                    + self.datetime_support.get_time_format_string()
                )
    ...
    def _build_tests_data_dict(self):
        ...
        if self.test.steps:
            self._get_steps_result_data(self.test, test_dict)
        return [test_dict]

    def _get_steps_result_data(self, test, test_dict):
        """
        ...
        for step in test.steps:
            ...
            if step.attachment:
                for attachment in step.attachment:
                    absolute = pathlib.Path().parent.absolute()
                    attachment.attachment_path = (
                        f"{absolute}/Tests/Reports/{story_id}/"
                        + attachment.attachment_path
                    )
        ...
        if update_examples:
            self._update_test_examples(test_dict, test.steps)
    ...
    def _request_query(self, request_type, query, **kwargs):
        """
