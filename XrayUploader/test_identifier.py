...
class TestIdentifier:
        ...
        for suite in self.suites:
            ...
            for test in suite.tests:
                ...
                if not story_jira_id:
                    self.logger.info(
                        "Jira id not available. Upload will not continue for test: %s",
                        test.name,
                    )
                    continue
                self.logger.info(
                    "Linked Jira ID(s) for Test are: %s", ",".join(story_jira_id)
                )
                try:
                    ...
                    self.logger.error(
                        "Failed to classify test under Jira ID: %s," + " testname: %s",
                        story_jira_id,
                        test.name,
                    )
        classification_dict[variables.DEPENDENCIES] = test_dependencies
    ...
    def _lookup_test_in_jira(self, test, story_jira_id):
        ...
        """
        self.logger.info(
            "Fetching linked tests to Jira Issue: %s", ",".join(story_jira_id)
        )
        ...
        test_category = variables.TEST_NOT_FOUND
        jira_test = None
        for linked_test in linked_tests:
            ...
            if jira_test_name == test.name:
                test_category = variables.TEST_NAME_FOUND
                jira_test = linked_test
                if self._compare_steps_same(test, linked_test):
                    test_category = variables.TEST_MATCHES_EXACTLY
    ...
    def _compare_steps_same(report_test, jira_test):
        ...
        """
        step_same = len(report_test.steps) == len(
            jira_test.fields.customfield_11404.steps
        )
        if step_same:
            jira_steps = jira_test.fields.customfield_11404.steps
            for counter, step in enumerate(report_test.steps):
                jira_test_description = getattr(
                    jira_steps[counter].fields, "Expected Result"
                )
                jira_test_result = getattr(jira_steps[counter].fields, "Action")
                if (
                    step.description != jira_test_description
                    or step.expected != jira_test_result
                ):
                    step_same = False
                    break
        return step_same
