...
class TestSuiteParser:
    ...
    def fetch_test_suite_data(self, dependencies):
        """
        ...
        suite_detail_dict = parse_keywords.get_step_details(
        ...
        suite = self._get_suite(
            suite_name,
            suite_doc,
            suite_details["jira_id"],
            suite_details["label_id"],
            setup_variables,
            dependencies,
        )
        return suite
    ...
    def _get_suite_details(self, suite_doc):
        ...
        :return: Dict of format: {"source": "", "suite_doc": "", "jira_id": "", "label_id": ""}
        """
        suite_source = self._get_suite_source()
        jira_id, suite_doc = self.soup_support.get_doc_extraction(
            JIRA_ID, suite_doc, ""
        )
        label_id, suite_doc = self.soup_support.get_doc_extraction(
            LABEL_ID, suite_doc, ""
        )
        suite_dict = {
            "source": suite_source,
