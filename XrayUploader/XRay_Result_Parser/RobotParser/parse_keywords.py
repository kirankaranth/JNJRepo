...
class ParseKeywords:
    ...
    def setup_variable_data(self):
        ...
        self.variables["Keyword_Name_Variables"] = self._get_variables_from_string(
            self.name
        )
        ...
        if arguments:
            for arg in arguments.find_all("arg"):
    ...
    def _add_all_message_recursively(self, step, messages):
        ...
        messages.extend(
            [
                msg.getText()
                for msg in step.findChildren("msg", recursive=False)
                if "<img src=" not in msg.getText()
            ]
        )
    ...
    def get_step_details(
        self, parent_keyword_parser=None, step_status="Passed", time_support=None
    ):
        """
        ...
        for keyword in self.keyword.findChildren("kw", recursive=False):
            ...
            inner_description = self._get_expected_details(
                _inner_keyword_parser
            ).strip()
            ...
            if _inner_keyword_parser.substituted_values and not actual.endswith(
        ...
        actual = self._add_all_messages_to_actual_if_fail(
            step_status, self.keyword, actual
        )
        return {
            "actual": actual,
            "expected": expected,
    ...
    def _check_var_type_not_in_parent_and_type_string(
        ...
        if var_type not in dict_parent[var_level]:
            if var_type == "STRING":
                dict_parent[var_level][var_type] = dict_child[var_level][var_type]
                check_continue = True
            else:
        ...
        for var_level in dict_child:
            ...
            for var_type in dict_child.get(var_level, {}):
                ...
                for variable in dict_child[var_level][var_type]:
                    dict_parent[var_level][var_type][variable] = dict_child[var_level][
                        var_type
                    ][variable]
    ...
    def _get_expected_details(keyword_parser):
        ...
        assign_variable = keyword_parser.variables.get("Assign_Variable", {}).get(
            "STRING", ""
        )
        if assign_variable:
            assign_variable += " = "
    ...
    def _get_variables_from_string(self, data_string):
        ...
        all_variables_start_location = [
            m.start()
            for m in re.finditer(parse_variables.ALL_VARIABLES_PATTERN, data_string)
        ]
        user_variables_start_location = [
            m.start()
            for m in re.finditer(parse_variables.USER_VAR_PATTERN, data_string)
        ]
        secured_user_variables_start_location = [
            m.start()
            for m in re.finditer(parse_variables.USER_VAR_PATTERN_SECURED, data_string)
        ]
        robot_variable_location = sorted(
            list(
                set(all_variables_start_location)
    ...
    def _build_variables_for_type(self, data_string, location, variable_dict):
        ...
        for counter, variable_start in enumerate(location):
            ...
            variable_end = self._find_variable_end(
                data_string, variable_start, end_position
            )
            if variable_end and variable_end > variable_start:
                variable_dict.update(
                    {data_string[variable_start : variable_end + 1]: ""}
                )
    ...
    def _find_variable_end(data, start, end):
        ...
        if variable_end_position > 0:
            ...
            if (
                len(data) > end_position + 1 and data[end_position + 1] == "["
            ):  # Dict variable
    ...
    def _get_variable_values(self):
        ...
        """
        keyword_messages = [
            x.string for x in self.keyword.findChildren("msg", recursive=False)
        ]
    ...
    def _find_value_for_assign_variable(self, keyword_message):
        ...
        if variable_dict:
            ...
            if variable_key + " = " in variable_value:
                variable_value = variable_value.split(variable_key + " = ", 1)[
                    1
                ].strip()
    ...
    def _find_variable_name(variable_format):
        ...
        secured_user_variables = re.findall(
            parse_variables.USER_VAR_PATTERN_SECURED, variable_format
        )
    ...
    def _match_user_variable_in_keyword_messages(
        self, user_vars_dict, keyword_messages
    ):
        """
    ...
    def _update_variables_with_matching_argument_length(
        ...
        if keyword_messages and len(keyword_messages) == len(robot_vars_dict):
            counter = 0
    ...
    @staticmethod
    def _update_innver_variable_dict(
        inner_variable_dict, user_vars_dict, robot_vars_dict
    ):
        """
        ...
        if inner_variable_dict:
            for user_variable in user_vars_dict:
                if user_variable in inner_variable_dict["Arguments"].get("USER", {}):
                    user_vars_dict[user_variable] = inner_variable_dict["Arguments"][
                        "USER"
                    ][user_variable]

            for user_variable in robot_vars_dict:
                if user_variable in inner_variable_dict["Arguments"].get("ROBOT", {}):
                    robot_vars_dict[user_variable] = inner_variable_dict["Arguments"][
                        "ROBOT"
                    ][user_variable]
    ...
    def _match_no_messages_to_inner_keywords(
        ...
        """
        if (
            (user_vars_dict or robot_vars_dict)
            and not keyword_messages
            and init_length
            and self.recursive
        ):
            ...
            self._update_innver_variable_dict(
                inner_variable_dict, user_vars_dict, robot_vars_dict
            )
    ...
    def _messages_and_arguments_length_differ(
        ...
        if 1 < len(keyword_messages) != total_variables and total_variables > 1:
            self._match_user_variable_in_keyword_messages(
                user_vars_dict, keyword_messages
            )
            if keyword_messages:
                counter = 0
                for robot_variable in robot_vars_dict:
            ...
            if variable_data_dict:
                for robot_variable in self.variables["Arguments"].get("ROBOT", {}):
                    if robot_variable in variable_data_dict:
                        self.variables["Arguments"]["ROBOT"][
                            robot_variable
    ...
    def _read_value_from_substrings(self, keyword_messages):
        ...
        find_variables_in_string = [
            True
            for x in list(variables_to_find.keys())
            if x in self.variables["Arguments"].get("STRING", "")
        ]
        self._update_variables_dict_if_length_match(
            find_variables_in_string,
            variables_to_find,
            keyword_messages,
            variable_data_dict,
    ...
    @staticmethod
    def _get_variable_mapping(variable_replaced_string, message, variable_list=None):
        """
        ...
        for tag, i1, i2, j1, j2 in seq_matcher.get_opcodes():
            ...
            if tag == "replace":
                if (
                    variable_replaced_string[i1:i2]
                    == parse_variables.VARIABLE_REPLACEMENT
                ):
