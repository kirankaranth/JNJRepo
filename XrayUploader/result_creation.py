...
class ResultCreation:
    ...
    def __init__(self, test_classification, args, jira, story_id, object_name):
        """
    ...
    def _ignore_tests_not_in_xray(self):
        ...
        """
        tests_not_in_xray = self.test_classification.get(variables.TEST_NOT_FOUND)
        if tests_not_in_xray:
            self.logger.info(
                "Following tests are not added to XRay currently "
    ...
    def _ignore_tests_not_reviewed(self):
        ...
        :returns: List of test which are in completed state and for which result
                    can be added.
        """
        test_differing_steps = self.test_classification.get(
            variables.TEST_NAME_FOUND, {}
        )
        all_tests = self.test_classification.get(variables.TEST_MATCHES_EXACTLY, {})
        all_tests.update(test_differing_steps)
        not_reviewed = []
        reviewed = []
        for test in all_tests:
            if all_tests[test].fields.status.name != variables.TEST_COMPLETED_STATUS:
                not_reviewed.append(test)
            else:
                reviewed.append(test)
        if not_reviewed:
            self.logger.info(
                "Following tests are not in status Completed in XRay currently "
                "and the results will not be added for them."
            )
        ...
        if not self.test_execution_issue:
            if not self.args.xray_testexecution_id and not self.args.no_jira_upload:
                self.test_execution_issue = self.jira.create_test_execution(
                    self.story_id, self.release, self.object_name
                )
    ...
    def _upload_reviewed_tests(self, reviewed_tests, classified_tests):
        ...
        """
        self.test_execution_dates = {
            variables.TEST_EXECUTION_FIRST_START: None,
            variables.TEST_EXECUTION_LAST_END: None,
        }
        uploaded = [True]
        for counter, test in enumerate(reviewed_tests):
            if self.args.dryrun:
                self.logger.info(
                    "%d. '%s' Result will not be added into Jira '%s'"
                    "for test id '%s'."
                ...
                self.jira.add_result(
                    test,
                    classified_tests[test],
                    self.test_execution_issue,
                    counter + 1,
                    uploaded=uploaded,
                )
        return uploaded[0]
    ...
    def _update_test_execution_details(self, uploaded, classified_tests):
        ...
        if self.args.xray_testexecution_id and uploaded:
            ...
            self.jira.update_issue_with_dict(
                self.test_execution_issue, self.args.tests_execution_fields
            )
            self.logger.info(
                "Test Execution ID for the Result: %s", self.args.xray_testexecution_id
            )
    ...
    def _get_updated_tests_from_classification(self, tests):
        ...
        """
        test_dict = {
            variables.DEPENDENCIES: self.test_classification[variables.DEPENDENCIES]
        }
        for test in tests:
