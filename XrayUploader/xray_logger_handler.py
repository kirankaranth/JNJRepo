...
def setup(results_file="", action="Test_Management_Uploader"):
    """
    Sets up logging for xray uploader
    :param results_file: Path to where the result file.
    :param action: The action being performed for xray uploader
    """
    global LOGGER
    if LOGGER:
        return LOGGER
    logger = logging.getLogger("Logger for XRay Uploader")
    ...
    file_path = os.sep.join([folder_location, action + "_XrayUploader.log"])
