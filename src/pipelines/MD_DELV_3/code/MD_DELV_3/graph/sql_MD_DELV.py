...
ON LIPS.VGBEL=VBAK.VBELN and VBAK._deleted_ = 'F' AND VBAK.MANDT = \"400\" where
LIPS._deleted_ = 'F' AND LIPS.MANDT = \"400\" and TRIM(LIPS.VGBEL) <> '')SUB_VBAK ON LIKP.VBELN=SUB_VBAK.VBELN
...
--ON LIPS.VGPOS=VBUK.VBELN and VBUK._deleted_ = 'F' AND VBUK.MANDT = \"400\" where LIPS._deleted_ = 'F' AND LIPS.MANDT = \"400\"
--) SUB_VBUK
--ON LIKP.VBELN=SUB_VBUK.VBELN
LEFT JOIN {Config.sourceDatabase}.VBUK VBUK ON LIKP.VBELN=VBUK.VBELN AND VBUK._deleted_ = 'F'  AND VBUK.MANDT = \"400\"
LEFT JOIN {Config.sourceDatabase}.TVKOT TVKOT ON LIKP.VKORG=TVKOT.VKORG AND TVKOT.SPRAS = 'E'  AND TVKOT._deleted_ = 'F' AND TVKOT.MANDT = \"400\"
LEFT JOIN {Config.sourceDatabase}.TVLS AS TVLS ON LIKP.LIFSK=TVLS.LIFSP  AND TVLS._deleted_ = 'F' AND TVLS.MANDT = \"400\"
LEFT JOIN {Config.sourceDatabase}.TVLST AS TVLST ON LIKP.LIFSK=TVLST.LIFSP AND TVLST.SPRAS = 'E' AND TVLST._deleted_ = 'F' AND TVLST.MANDT = \"400\"
LEFT JOIN {Config.sourceDatabase}.TVSTT AS TVSTT ON LIKP.VSTEL=TVSTT.VSTEL  AND TVSTT.SPRAS = 'E' AND TVSTT._deleted_ = 'F' AND TVSTT.MANDT = \"400\"
LEFT JOIN {Config.sourceDatabase}.TVRO AS TVRO ON LIKP.ROUTE=TVRO.ROUTE AND TVRO._deleted_ = 'F' AND TVRO.MANDT = \"400\"
LEFT JOIN {Config.sourceDatabase}.TVROT AS TVROT ON LIKP.ROUTE=TVROT.ROUTE AND TVROT.SPRAS = 'E' AND TVROT._deleted_ = 'F' AND TVROT.MANDT = \"400\"
WHERE LIKP._deleted_ = 'F' AND LIKP.MANDT = \"400\"
