...
def NEW_FIELDS(spark: SparkSession, in0: DataFrame) -> DataFrame:
    return in0\
        ...
        .withColumn("SD_UNIQ_DOC_RL_ID", col("RUUID"))\
        ...
        .withColumn(
          ...
          when(
              (
                ((col("_DATAAGING") == lit("00000000")) | (col("_DATAAGING") < lit("19000101")))
                | (
                  (length(regexp_replace(col("_DATAAGING"), "(\\d+)", "")) > lit(0))
                  | (length(col("_DATAAGING")) < lit(8))
                )
              ), 
              lit(None)
            )\
