...
TRIM(TTSGT.BEZEI) as SHIP_BLOK_RSN_DESC,
...
ON LIPS.VGBEL=VBAK.VBELN and VBAK._deleted_ = 'F' AND VBAK.MANDT = \"232\" where
LIPS._deleted_ = 'F' AND LIPS.MANDT = \"232\" and TRIM(LIPS.VGBEL) <> '')SUB_VBAK ON LIKP.VBELN=SUB_VBAK.VBELN
LEFT JOIN {Config.sourceDatabase}.TVLKT TVLKT ON LIKP.LFART=TVLKT.LFART AND TVLKT.SPRAS = 'E' AND TVLKT._deleted_ = 'F' AND TVLKT.MANDT = \"232\"
...
--ON LIPS.VGPOS=VBUK.VBELN and VBUK._deleted_ = 'F' AND VBUK.MANDT = \"232\" where LIPS._deleted_ = 'F' AND LIPS.MANDT = \"232\"
--) SUB_VBUK
--ON LIKP.VBELN=SUB_VBUK.VBELN
LEFT JOIN {Config.sourceDatabase}.VBUK VBUK ON LIKP.VBELN=VBUK.VBELN AND VBUK._deleted_ = 'F'  AND VBUK.MANDT = \"232\"
LEFT JOIN {Config.sourceDatabase}.TVKOT TVKOT ON LIKP.VKORG=TVKOT.VKORG AND TVKOT.SPRAS = 'E'  AND TVKOT._deleted_ = 'F' AND TVKOT.MANDT = \"232\"
LEFT JOIN {Config.sourceDatabase}.TVLS AS TVLS ON LIKP.LIFSK=TVLS.LIFSP AND TVLS._deleted_ = 'F' AND TVLS.MANDT = \"232\"
LEFT JOIN {Config.sourceDatabase}.TVLST AS TVLST ON LIKP.LIFSK=TVLST.LIFSP AND TVLST.SPRAS = 'E' AND TVLST._deleted_ = 'F' AND TVLST.MANDT = \"232\"
LEFT JOIN {Config.sourceDatabase}.TVSTT AS TVSTT ON LIKP.VSTEL=TVSTT.VSTEL  AND TVSTT.SPRAS = 'E' AND TVSTT._deleted_ = 'F' AND TVSTT.MANDT = \"232\"
LEFT JOIN {Config.sourceDatabase}.TVRO AS TVRO ON LIKP.ROUTE=TVRO.ROUTE AND TVRO._deleted_ = 'F' AND TVRO.MANDT = \"232\"
LEFT JOIN {Config.sourceDatabase}.TVROT AS TVROT ON LIKP.ROUTE=TVROT.ROUTE AND TVROT.SPRAS = 'E' AND TVROT._deleted_ = 'F' AND TVROT.MANDT = \"232\"
LEFT JOIN {Config.sourceDatabase}.TTSGT AS TTSGT ON LIKP.TRSPG=TTSGT.TRSPG AND TTSGT.SPRAS = 'E' AND TTSGT._deleted_ = 'F' AND TTSGT.MANDT = \"232\"
WHERE LIKP._deleted_ = 'F' AND LIKP.MANDT = \"232\"
