...
def Join_1(spark: SparkSession, VBAP: DataFrame, VBAK: DataFrame, VBKD: DataFrame) -> DataFrame:
    return VBAP\
        .alias("VBAP")\
        ...
        when(
            (
              (col("VBAP.session_creation_date") == lit("00000000"))
              | (col("VBAP.session_creation_time") == lit("00000000"))
            ), 
            lit(None).cast(TimestampType())
          )\
          .otherwise(to_timestamp(concat(col("VBAP.session_creation_date"), col("VBAP.session_creation_time")), "yyyyMMddHHmmss"))\
