...
from sap_01_md_sls_ordr_line_geu.graph import *
...
def pipeline(spark: SparkSession) -> None:
    ...
    df_SAP_TVM4T = collectMetrics(
        ...
        df_SAP_TVM4T, 
        "graph", 
        "Zjl4eZKkzNt4ppcNTiYfN$$aDlrjWmCf9m-b8qomuDjj", 
        "42Sxc6FHsS82IZu8nu-Vt$$bX5wCybWoyRCJBvEJ2L4q"
    )
    df_MANDT_FILTER_TVM4T = MANDT_FILTER_TVM4T(spark, df_SAP_TVM4T)
    LU_SAP_TVM3T_1(spark, df_MANDT_FILTER_TVM4T)
...
def main():
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri", "pipelines/SAP_01_MD_SLS_ORDR_LINE_geu")
    
    MetricsCollector.start(spark = spark, pipelineId = "pipelines/SAP_01_MD_SLS_ORDR_LINE_geu")
    pipeline(spark)
