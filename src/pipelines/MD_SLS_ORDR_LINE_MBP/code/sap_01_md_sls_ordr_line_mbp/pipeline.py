...
from sap_01_md_sls_ordr_line_mbp.graph import *
...
def pipeline(spark: SparkSession) -> None:
    ...
    df_SAP_TVM4T = collectMetrics(
        ...
        df_SAP_TVM4T, 
        "graph", 
        "Wn83mPiQhLjoqphWbqML0$$w4dHWFkGSqYFHbr-ADSry", 
        "VhTCDmrMuXKQFj11ySYVf$$ZBkVTq1wp6-NFOO3LwJxB"
    )
...
def main():
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri", "pipelines/MD_SLS_ORDR_LINE_MBP")
    
    MetricsCollector.start(spark = spark, pipelineId = "pipelines/MD_SLS_ORDR_LINE_MBP")
    pipeline(spark)
