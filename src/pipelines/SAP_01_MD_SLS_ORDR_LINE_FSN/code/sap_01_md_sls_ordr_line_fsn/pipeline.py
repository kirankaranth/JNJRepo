...
from sap_01_md_sls_ordr_line_fsn.graph import *
...
def pipeline(spark: SparkSession) -> None:
    ...
    df_SAP_TVM4T = collectMetrics(
        ...
        df_SAP_TVM4T, 
        "graph", 
        "qnSAx-Ke7e05cfFa8C70b$$-UNQZnLlg-OcspYPiosNA", 
        "FvrT3uXG6MVOnVY8FK6JJ$$iiJAT4IyErZhaYyBrW3Fp"
    )
    df_Filter_1_TVM3T = Filter_1_TVM3T(spark, df_SAP_TVM4T)
    LU_SAP_TVM4T(spark, df_Filter_1_TVM3T)
...
def main():
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri", "pipelines/SAP_01_MD_SLS_ORDR_LINE_FSN")
    
    MetricsCollector.start(spark = spark, pipelineId = "pipelines/SAP_01_MD_SLS_ORDR_LINE_FSN")
    pipeline(spark)
