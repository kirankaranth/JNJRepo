...
def NEW_FIELDS_RENAME_FORMAT(spark: SparkSession, in0: DataFrame) -> DataFrame:
    return in0\
        ...
        .withColumn("STK_SGMNT", trim(col("SGT_SCAT")))\
        .withColumn("CUST_NUM1", trim(col("KUNNR2")))\
        .withColumn("SEASN_YR", trim(col("FSH_SEASON_YEAR")))\
        .withColumn("SEASN", trim(col("FSH_SEASON")))\
        .withColumn("FSHN_CLCT", trim(col("FSH_COLLECTION")))\
        .withColumn("FSHN_THEME", trim(col("FSH_THEME")))\
        .withColumn("ALC_STK_QTY", col("FSH_SALLOC_QTY").cast(DecimalType(18, 4)))\
        .withColumn("NUM_OF_ORIG_ORDR", trim(col("MILL_OC_AUFNR_U")))\
        .withColumn("CNFRM_QTY_FOR_ITM", col("MILL_OC_RUMNG").cast(DecimalType(18, 4)))\
        .withColumn("ITM_SEQ", trim(col("MILL_OC_SORT")))\
