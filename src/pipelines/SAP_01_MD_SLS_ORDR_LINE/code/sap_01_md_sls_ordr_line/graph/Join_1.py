...
def Join_1(
    ...
    return VBAP\
        ...
        trim(col("VBAP.KBMENG")).alias("CNFRM_QTY"), 
        trim(col("KWMENG")).alias("SLS_UNIT_ORDR_QTY"), 
        trim(col("VBAP.LFREL")).alias("DELV_RLVNT_IND"), 
        trim(col("VBAP.LGORT")).alias("SLOC_CD"), 
        trim(col("VBAP.MATNR")).alias("MATL_NUM"), 
        trim(col("VBAP.NETWR")).alias("NET_VAL_AMT"), 
        ...
        trim(col("VBAK.FMBDAT")).alias("ORG_MTL_AVAL_DTTM"), 
        ...
        trim(col("VBAP.NETPR")).alias("NET_PRC_AMT"), 
        trim(col("VBAP.MATWA")).alias("ENT_MATL_NUM"), 
        trim(col("VBAP.MEINS")).alias("BASE_UOM_CD"), 
        trim(col("VBKD.BSTKD")).alias("CUST_PO_NUM"), 
        trim(col("VBAP.UMVKZ")).alias("FCTR_NMRTR_MEAS"), 
        trim(col("VBAP.UMVKN")).alias("FCTR_DNMNTR_MEAS"), 
        trim(col("VBAP.LSMENG")).alias("SLS_VOL_QTY"), 
        trim(col("VBAP.BRGEW")).alias("GRS_WT_MEAS"), 
        trim(col("VBAP.GEWEI")).alias("WT_UOM_CD"), 
        trim(col("VBAP.NTGEW")).alias("NET_WT_MEAS"), 
        trim(col("VBAP.KDMAT")).alias("MATL_NUM_USED_BY_CUST"), 
        trim(col("VBAP.KLMENG")).alias("CUM_CNFRM_QTY_BASE_UNIT"), 
        ...
        trim(col("VBAP.VOLUM")).alias("VOL_MEAS"), 
        trim(col("VBAP.WAVWR")).alias("COST_IN_DOC_CRNCY"), 
        trim(col("VBAP.UMZIZ")).alias("FACT_FOR_CNV_SLS_UNIT_TO_BASE_UNIT"), 
        trim(col("VBAP.ARKTX")).alias("MATL_SHRT_DESC"), 
        trim(col("VBAP.CHARG")).alias("BTCH_NUM"), 
        trim(col("VBAP.LFMNG")).alias("MIN_DELV_QTY"), 
        ...
        when((col("VBAP.aedat") == lit("00000000")), lit(None).cast(TimestampType()))\
          ...
          .alias("CHG_DTTM"), 
        trim(col("VBAP.KZWI2")).alias("SBTOT_2_PRC_PCDR"), 
        trim(col("VBAP.KZWI3")).alias("SBTOT_3_PRC_PCDR"), 
        trim(col("VBAP.KZWI4")).alias("SBTOT_4_PRC_PCDR"), 
        trim(col("VBAP.KZWI5")).alias("SBTOT_5_PRC_PCDR"), 
        trim(col("VBAP.KZWI6")).alias("SBTOT_6_PRC_PCDR"), 
        trim(col("VBAP.AWAHR")).alias("ORDR_PRBLTY_ITM"), 
        trim(col("VBAP.SHKZG")).alias("RTRN_ITM"), 
        trim(col("VBAP.SKTOF")).alias("DR_CR_IN"), 
        trim(col("VBAP.ZMENG")).alias("TRGT_QTY_SLS_UNIT"), 
        ...
        trim(col("VBAP.KPEIN")).alias("COND_PRC_UNIT"), 
        ...
        trim(col("TVST.LOADTG")).alias("LD_TIME_WRK_DAYS"), 
        trim(col("TVST.PIPATG")).alias("PICK_PACK_TIME_WRK_DAYS"), 
        ...
        when((trim(col("VBKD.bstdk")) == lit("00000000")), lit(None).cast(TimestampType()))\
          ...
          .alias("CUST_PO_DTTM"), 
        trim(col("VBAP.UMZIN")).alias("CONV_FACT"), 
        ...
        trim(col("VBAP.KZWI1")).alias("SBTOT_1_PRC_PCDR"), 
        trim(col("VBAP.STCUR")).alias("STATS_EXCH_RT"), 
        ...
        trim(col("VBAP.CMPRE")).alias("ITM_CR_PRC_CMPRE"), 
        ...
        trim(col("VBAP.ZWERT")).alias("TRGT_VAL_OUTLN_AGMT_DOC_CRNCY"), 
        trim(col("VBAP.SMENG")).alias("SCALE_QTY_BASE_UNIT_MEAS"), 
        trim(col("VBAP.ABLFZ")).alias("RD_QTY_FOR_DELV"), 
        ...
        trim(col("VBAP.ABSFZ")).alias("ALLW_DEVT_QTY_ABS"), 
        ...
        trim(col("VBAP.UEBTO")).alias("OVR_DELV_TLRNC"), 
        trim(col("VBAP.UNTTO")).alias("UND_DELV_TLRNC"), 
        ...
        trim(col("VBAP.VBEAF")).alias("FX_SHIPPING_PRCSG_TIME_IN_DAYS"), 
        trim(col("VBAP.VBEAV")).alias("VAR_SHIPPING_PRCSG_TIME_IN_DAYS"), 
        ...
        trim(col("VBAP.KMPMG")).alias("CMPNT_QTY"), 
        ...
        trim(col("VBAP.UMREF")).alias("CONV_FACT_QTY"), 
        ...
        trim(col("VBAP.CMKUA")).alias("CR_DATA_EXCH_RT_FOR_RQST_DELV_DT"), 
        ...
        trim(col("VBAP.CMPRE_FLT")).alias("ITM_CR_PRC_CMPRE_FLT"), 
        trim(col("VBAP.ABFOR")).alias("FORM_PMT_GUAR"), 
        trim(col("VBAP.ABGES")).alias("GUARNTD"), 
        ...
        trim(col("VBAP.MWSBP")).alias("TAX_AMT_DOC_CRNCY"), 
        ...
        trim(col("VBAP._BITMYM_AVALUE")).alias("EQMNT_RSDL_VAL"), 
