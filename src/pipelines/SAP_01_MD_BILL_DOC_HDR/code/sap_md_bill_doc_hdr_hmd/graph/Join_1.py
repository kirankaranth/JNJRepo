...
def Join_1(
    ...
    df1 = in1.hint("broadcast")
    df2 = in2.hint("broadcast")
    df3 = in3.hint("broadcast")
    df4 = in4.hint("broadcast")

    return in0\
        .alias("in0")\
        .join(df1.alias("in1"), (col("in0.SPART") == col("in1.SPART")), "left_outer")\
        .join(df2.alias("in2"), (col("in0.FKART") == col("in2.FKART")), "left_outer")\
        .join(df3.alias("in3"), (col("in0.VKORG") == col("in3.VKORG")), "left_outer")\
        .join(df4.alias("in4"), (col("in0.VTWEG") == col("in4.VTWEG")), "left_outer")\
