...
CASE WHEN LOWER(TRIM(f0401_adt.a6upmj)) = 'null' OR TRIM(f0401_adt.a6upmj) = '' OR TRIM(f0401_adt.a6upmj) = '0'
THEN CAST(NULL AS TIMESTAMP) ELSE to_timestamp(substr(CAST(date_add(concat(substr(CAST(CAST(TRIM(f0401_adt.a6upmj) AS INT) + 1900000 AS STRING),1,4),'-01-01'), CAST(substr(CAST(CAST(TRIM(f0401_adt.a6upmj) AS INT) + 1900000 AS string),5) AS INT )-1) AS STRING), 1, 10),'yyyy-MM-dd') END AS CRT_ON_DTTM,
NULL AS PSTNG_BLK_IND,
NULL AS DEL_IND,
NULL AS LDGR_ACCT_CD,
trim(f0401_adt.a6pyin) AS PMT_METH_CD,
trim(f0401_adt.a6hdpy) AS PMT_BLK_IND,
trim(f0401_adt.a6trap) AS PMT_TERM_CD,
