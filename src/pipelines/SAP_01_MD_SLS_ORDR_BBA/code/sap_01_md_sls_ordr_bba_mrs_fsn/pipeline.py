...
from sap_01_md_sls_ordr_bba_mrs_fsn.graph import *
...
def pipeline(spark: SparkSession) -> None:
    ...
    df_SAP_TVV3T = collectMetrics(
        spark, 
        df_SAP_TVV3T, 
        "graph", 
        "SWhzeqbe19FGvoi4-1npA$$nnT4JFtQ6kVbYYcGVhLSw", 
        "-DJqdAWjBALLMZHqHYfK2$$6ZHydEdIp8TJi_lWaQM6j"
    )
...
def main():
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri", "pipelines/SAP_01_MD_SLS_ORDR_BBA")
    
    MetricsCollector.start(spark = spark, pipelineId = "pipelines/SAP_01_MD_SLS_ORDR_BBA")
    pipeline(spark)
