...
def NEW_FIELDS_TRANSFORMATION(spark: SparkSession, in0: DataFrame) -> DataFrame:
    return in0\
        ...
        .withColumn("EXTN_ENABL_CO_CD", trim(col("KNB1_EEW_CC")))\
        .withColumn(
          "REC_CRT_DTTM",
          when(
              (
        ...
        .withColumn("IN_SEND_PMT_ADVC_XML", trim(col("AVSND")))\
        .withColumn("EMAIL_ADDR_HASH_VAL", trim(col("AD_HASH")))\
        .withColumn("WTHL_TAX_CTRY_RGN_KEY", trim(col("QLAND")))\
        .withColumn("BUSN_PRPS_CMPLT_FL", trim(col("CVP_XBLCK_B")))\
        .withColumn("MN_ECON_ACTV", trim(col("CIIUCODE")))\
        .withColumn("PMT_CLRNG_GRP_ID", trim(col("PAYMENTCLEARINGGRPID")))\
