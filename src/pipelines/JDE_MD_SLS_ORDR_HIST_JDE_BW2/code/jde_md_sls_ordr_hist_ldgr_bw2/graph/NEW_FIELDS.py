...
def NEW_FIELDS(spark: SparkSession, in0: DataFrame) -> DataFrame:
    return in0\
        ...
        .withColumn("MFG_VAR_ACTG_FL", trim(col("SLSO16")))\
        .withColumn("SLS_ORDR_STS_17", trim(col("SLSO17")))\
        .withColumn("SLS_ORDR_STS_18", trim(col("SLSO18")))\
        .withColumn("SLS_ORDR_STS_19", trim(col("SLSO19")))\
        .withColumn("SLS_ORDR_STS_20", trim(col("SLSO20")))\
        .withColumn("INTGR_REF_01", trim(col("SLIR01")))\
        .withColumn("INTGR_REF_02", trim(col("SLIR02")))\
        .withColumn("INTGR_REF_03", trim(col("SLIR03")))\
        .withColumn("INTGR_REF_04", trim(col("SLIR04")))\
        .withColumn("INTGR_REF_05", trim(col("SLIR05")))\
        .withColumn("SRC_OF_ORDR", trim(col("SLSOOR")))\
        .withColumn("REF_3", trim(col("SLVR03")))\
        .withColumn("DMAND_UNIQ_KEY_ID", trim(col("SLDEID")))\
        .withColumn("PULL_SIGNL", trim(col("SLPSIG")))\
        .withColumn("RLSE_NUM", trim(col("SLRLNU")))\
        .withColumn("SCHD_SHIP_TIME", trim(col("SLPMDT")))\
        .withColumn("TIME_RLSE", trim(col("SLRLTM")))\
        .withColumn(
          "RLSE_DTTM",
          to_timestamp(
            date_add(
              substring((col("SLRLDJ") + lit(1900000)), 1, 4).cast(DateType()), 
              (substring(col("SLRLDJ"), 4, 3).cast(IntegerType()) - 1)
            )
          )
        )\
        ...
        .withColumn("TIME_FUT_TIME_2", trim(col("SLPSTM")))\
        .withColumn("CROSS_DK_FL", trim(col("SLXDCK")))\
        .withColumn("CROSS_DK_PRIR_FOR_SLS_ORD", trim(col("SLXPTY")))\
        .withColumn("DUAL_UNIT_OF_MEAS_ITM", trim(col("SLDUAL")))\
        .withColumn("BUY_SGMNT_CD", trim(col("SLBSC")))\
        .withColumn("CUR_BUY_SGMNT_CD", trim(col("SLCBSC")))\
        .withColumn("CHG_ORDR_NUM", trim(col("SLCORD")))\
        .withColumn("ADDR_NUM_DELV_TO", trim(col("SLDVAN")))\
        .withColumn("PEND_APPR_FL", trim(col("SLPEND")))\
        .withColumn("RVSN_RSN", trim(col("SLRFRV")))\
        .withColumn("MATRX_CNTL_LINE_NUM", trim(col("SLMCLN")))\
        .withColumn("SHIP_NUM", trim(col("SLSHPN")))\
        .withColumn("PROM_DELV_TIME", trim(col("SLRSDT")))\
        .withColumn("PROJ_NUM", trim(col("SLPRJM")))\
        .withColumn("SEQ_NUM", trim(col("SLOSEQ")))\
        .withColumn("ITM_RVSN_LVL", trim(col("SLMERL")))\
        .withColumn("HOLD_ORD_CD", trim(col("SLHOLD")))\
        .withColumn("BUSN_UNIT_HDR_2", trim(col("SLHDBU")))\
        .withColumn("BUSN_UNIT_DMAND", trim(col("SLDMBU")))\
        .withColumn("CRNCY_CD_BASE", trim(col("SLBCRC")))\
        .withColumn("DOC_LINE_NUM_ORIG", trim(col("SLODLN")))\
        .withColumn(
          "ORIG_PROM_DELV_DTTM",
          to_timestamp(
            date_add(
        ...
        .withColumn("CROSS_DK_ORDR_CO", trim(col("SLXKCO")))\
        .withColumn("CROSS_DK_ORDR_NO", trim(col("SLXORN")))\
        .withColumn("CROSS_DK_ORDR_TYPE", trim(col("SLXCTO")))\
        .withColumn("CROSS_DK_LINE_NUM", trim(col("SLXLLN")))\
        .withColumn("CROSS_DK_ORDR_SFX", trim(col("SLXSFX")))\
        .withColumn("PORT_OF_ENT_OR_EXIT", trim(col("SLPOE")))\
        .withColumn("PMT_TERM_OVRD_CD", trim(col("SLPMTO")))\
        .withColumn("BUY_NUM", trim(col("SLANBY")))\
        .withColumn("PROM_ID", trim(col("SLPMTN")))\
        .withColumn("ASSET_ITM_NUM", trim(col("SLNUMB")))\
        .withColumn("PARNT_NUM", trim(col("SLAAID")))\
        .withColumn("PTNR_ADDR_NUM", trim(col("SLPRAN8")))\
        .withColumn("SHIP_TO_ATTN", trim(col("SLSPATTN")))\
        .withColumn("PTNR_CNTCT_LINE_NUM_ID", trim(col("SLPRCIDLN")))\
        .withColumn("CUST_CNTCT_LINE_NUM_ID", trim(col("SLCCIDLN")))\
        .withColumn("SHIP_TO_CUST_CNTCT_LINE_NUM_ID", trim(col("SLSHCCIDLN")))\
        .withColumn("OPP_NUM", trim(col("SLOPPID")))\
        .withColumn("ORG_STRC_TYPE", trim(col("SLOSTP")))\
        .withColumn("UNIQ_KEY_ID", trim(col("SLUKID")))\
        .withColumn("CATLG_NM", trim(col("SLCATNM")))\
        .withColumn("ALLC_PRCS_FL", trim(col("SLALLOC")))\
        .withColumn("FFL_PLAN_ID", trim(col("SLFULPID")))\
        .withColumn("ALLC_STS_NUM", trim(col("SLALLSTS")))\
        .withColumn("SLS_ORDR_SCORE", trim(col("SLOSCORE")))\
        .withColumn("ORDR_SCORE_OVRD", trim(col("SLOSCOREO")))\
        .withColumn("CUST_MSTR_CO", trim(col("SLCMCO")))\
        .withColumn("KIT_ID", trim(col("SLKITID")))\
        .withColumn("ACUM_AMT_INVC_DOM", trim(col("SLKITAMTDOM")).cast(DecimalType(18, 4)))\
        .withColumn("ACUM_AMT_INVC_FRGN", trim(col("SLKITAMTFOR")).cast(DecimalType(18, 4)))\
        .withColumn("KIT_CMPNT_DIRTY_FL", trim(col("SLKITDIRTY")))\
        .withColumn("OC_IN_TRNST_FL", trim(col("SLOCITT")))\
        .withColumn("OC_KANBAN_CARD_NO", trim(col("SLOCCARDNO")))\
        .withColumn("PRDTN_NUM", trim(col("SLPMPN")))\
        .withColumn("PRDTN_NUM_SHRT", trim(col("SLPNS")))\
